...
func TestNewBasicInterpreter(t *testing.T) {
	_, err := NewBasicInterpreter(NewMockCanvas)
	if err != nil {
		t.Errorf("Expected: err == nil, Got: %#v", err)
	}

	_, err = NewBasicInterpreter(nil)
...
func TestBasicInterpreter_Interpret(t *testing.T) {
	// Positive Cases
	interpPos, err := NewBasicInterpreter(NewMockCanvas)
	if err != nil {
		panic(err)
	}
	...
	casesPos := []struct {
		...
		{command.BucketFillCommand{X: 10, Y: 3, C: color.ByteColor('o')}, command.BucketFillCommand{X: 9, Y: 2, C: color.ByteColor('o')}}, // Example 5
	}
	...
	// Negative Cases
	interpNeg, err := NewBasicInterpreter(func(width, height int) (canvas.Canvas, error) {
		return canvas.NewByteColorBuffer(width, height, color.ByteColor(' '), color.ByteColor('x'))
	})
	if err != nil {
		panic(err)
	}
	...
	casesNeg := []struct {
		...
		{command.BucketFillCommand{X: 10, Y: 3, C: color.ByteColor('o')}, common.ErrCanvasNotCreated},
		...
		{command.BucketFillCommand{X: -1, Y: -1, C: color.ByteColor('o')}, common.ErrPointOutsideCanvas},
	}
